...
export default function LeaveReports() {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      const generatedByText = `Generated by: ${user?.username || "System"}`;
      const generatedbyWidth = doc.getTextWidth(generatedByText);
      const generatedbyX = (pageWidth - generatedbyWidth) / 2;
      doc.text(generatedByText, generatedbyX, 40);

      const generatedText = `Generated on: ${new Date().toLocaleDateString()}`;
      const generatedWidth = doc.getTextWidth(generatedText);
      const generatedX = (pageWidth - generatedWidth) / 2;
      doc.text(generatedText, generatedX, 50);
      ...
      doc.text(recordsText, recordsX, 60);
